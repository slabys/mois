...
import type { GenerateInvoice, GenerateInvoiceResult } from "apps/documents/src/types";
...
@Injectable()
export class InvoiceService implements OnApplicationBootstrap, OnApplicationShutdown {
	constructor(
		@Inject("DOCUMENTS")
	...
	private formatInvoiceId(number: number, year: number) {
		const suffix = number.toString().length > 4 ? number : "0".repeat(4 - number) + number;
	...
	generatePdf(data: GenerateInvoice["data"], force?: boolean) {
		...
		const observable = this.clientProxy.send<GenerateInvoiceResult>("invoice.generate", payload);
	...
	generatePdfFromInvoice(invoice: Invoice, force?: boolean) {
		const totalCost = invoice.items.reduce((prev, current) => prev + current.price, 0);
