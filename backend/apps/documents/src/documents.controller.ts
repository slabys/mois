...
export class DocumentsController {
	...
	async invoiceGenerate(@Payload() { data, outputPath, force }: GenerateInvoice): Promise<GenerateInvoiceResult> {
		/**
		 * TODO: Handle errors or create app-wise interceptor
		 */
		if (!force && (await this.storageService.exist(outputPath))) return { success: true };
