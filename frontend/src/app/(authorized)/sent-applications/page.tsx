...
import type { EventApplicationSimpleWithApplications } from "@/utils/api.schemas";
      ...
      <Stack>
        ...
        {upcomingEvents && (
          <Stack>
            ...
            {(upcomingEvents as unknown as EventApplicationSimpleWithApplications[])?.length >= 1 ? (
              (upcomingEvents as unknown as EventApplicationSimpleWithApplications[])?.map(
                (eventApplication, index) => (
                  <Anchor
                    component={Link}
        ...
        {pastEvents && (
          <Stack>
            ...
            {(pastEvents as unknown as EventApplicationSimpleWithApplications[])?.length >= 1 ? (
              (pastEvents as unknown as EventApplicationSimpleWithApplications[])?.map((eventApplication, index) => (
                <Anchor
                  component={Link}
                  key={`event-card-${index}-${eventApplication.id}`}
                  href={routes.EVENT_DETAIL({ id: eventApplication.event.id })}
